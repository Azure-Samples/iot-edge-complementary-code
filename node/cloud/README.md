...
2. shared - This folder contains a *compression* node package, which also contains a unit test folder (*compression/test*).  The *compression* package is used by both the Azure Functions App project in this workspace and the Azure IoT Edge solution in the *edge* workspace. The *compression* package is included to demonstrate source code sharing between edge and cloud code.  It actually uses the [node *zlib* package](https://nodejs.org/dist/latest/docs/api/zlib.html) to do the compression.
